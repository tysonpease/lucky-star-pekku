...
def add_base_tags_to_channel(channel, comic_url, comic_info):
    ...
    ElementTree.SubElement(channel, "title").text = comic_info.get(
        "Comic Info", "Comic name"
    )
    ElementTree.SubElement(channel, "description").text = comic_info.get(
        "RSS Feed", "Description"
    )
    ElementTree.SubElement(channel, "link").text = comic_url
    ElementTree.SubElement(
        channel, "{http://purl.org/dc/elements/1.1/}creator"
    ).text = comic_info.get("Comic Info", "Author")
    ElementTree.SubElement(channel, "language").text = comic_info.get(
        "RSS Feed", "Language"
    )
...
def add_image_tag(channel, comic_url, comic_info):
    ...
    ElementTree.SubElement(image_tag, "title").text = comic_info.get(
        "Comic Info", "Comic name"
    )
    ElementTree.SubElement(image_tag, "link").text = comic_url
    ElementTree.SubElement(image_tag, "url").text = urljoin(
        comic_url, comic_info.get("RSS Feed", "Image")
    )
    ElementTree.SubElement(image_tag, "width").text = comic_info.get(
        "RSS Feed", "Image width"
    )
    ElementTree.SubElement(image_tag, "height").text = comic_info.get(
        "RSS Feed", "Image height"
    )
...
def add_item(xml_parent, comic_data, comic_url, comic_info):
    ...
    post_date = strptime(
        comic_data["post_date"], comic_info.get("Comic Settings", "Date format")
    )
    ElementTree.SubElement(item, "pubDate").text = strftime(
        "%a, %d %b %Y %H:%M:%S +0000", post_date
    )
    ...
    if "tags" in comic_data:
        for tag in comic_data["tags"]:
            ...
            e.attrib["type"] = "tag"
            e.text = tag
    comic_image_url = urljoin(
        comic_url, "your_content/comics/{}/{}".format(post_id, comic_data["filename"])
    )
    html = build_rss_post(
        comic_image_url, comic_data.get("alt_text"), comic_data["post_html"]
    )
