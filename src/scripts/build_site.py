...
def get_links_list(comic_info: RawConfigParser):
    ...
    for option in comic_info.options("Links Bar"):
        link_list.append(
            {"name": option, "url": web_path(comic_info.get("Links Bar", option))}
        )
    return link_list
        ...
        return [
            {
                ...
                "title": web_path(comic_info.get(section_name, option)),
            }
...
def build_and_publish_comic_pages(
    ...
    processing_times.append(
        (f"Save page_info_list.json file in '{comic_folder}'", time())
    )
    ...
    processing_times.append(
        (f"Build full comic data dicts for '{comic_folder}'", time())
    )
    ...
    global_values = {
        ...
        "use_images_in_navigation_bar": comic_info.getboolean(
            "Comic Settings", "Use images in navigation bar"
        ),
        ...
        "google_analytics_id": get_option(
            comic_info, "Google Analytics", "Tracking ID", default=""
        ),
...
def get_page_info_list(
    ...
    for page_path in glob(f"your_content/{comic_folder}comics/*/"):
        ...
        post_date = tz_info.localize(
            datetime.strptime(page_info["Post date"], date_format)
        )
            ...
            if not page_info.get("Filename", ""):
                if not auto_detect_comic_images:
                    raise FileNotFoundError(
                        f"Comic image filename must be provided in {page_path}info.ini"
                    )
                image_files = []
                for filename in os.listdir(page_path):
                    if filename == "thumbnail.jpg":
                        continue
                    if re.search(
                        r"\.(jpg|jpeg|png|tif|tiff|gif|bmp|webp|webv|svg|eps)$",
                        filename,
                    ):
...
def get_transcripts(
    comic_folder: str, comic_info: RawConfigParser, page_name: str
) -> OrderedDict:
    ...
    if get_option(
        ...
        load_transcripts_from_folder(
            transcripts, f"your_content/{comic_folder}comics", page_name
        )
    transcripts_dir = get_option(
        comic_info, "Transcripts", "Transcripts folder", default=f""
    )
    if transcripts_dir:
        load_transcripts_from_folder(transcripts, transcripts_dir, page_name)
    default_language = get_option(
        comic_info, "Transcripts", "Default language", default=f"English"
    )
...
def load_transcripts_from_folder(
    transcripts: OrderedDict, transcripts_dir: str, page_name: str
):
    for transcript_path in sorted(
        glob(os.path.join(transcripts_dir, page_name, "*.txt"))
    ):
...
def create_comic_data(
    ...
    archive_post_date = strftime(
        ...
        strptime(
            page_info["Post date"], comic_info.get("Comic Settings", "Date format")
        ),
    )
    post_html = []
    post_text_paths = [
    ...
    d = {
        ...
        "transcripts": get_transcripts(
            comic_folder, comic_info, page_info["page_name"]
        ),
    }
    theme = get_option(comic_info, "Comic Settings", "Theme", default="default")
    hook_result = run_hook(
        theme, "extra_comic_dict_processing", [comic_folder, comic_info, d]
    )
    if hook_result:
        d = hook_result
    print(d)
...
def build_comic_data_dicts(
    comic_folder: str, comic_info: RawConfigParser, page_info_list: List[Dict]
) -> List[Dict]:
    return [
        create_comic_data(
...
def process_comic_image(comic_info, comic_page_path):
    ...
    comic_page_name, comic_page_ext = os.path.splitext(
        os.path.basename(comic_page_path)
    )
    with open(comic_page_path, "rb") as f:
        im = Image.open(f)
        thumbnail_path = os.path.join(comic_page_dir, "thumbnail.jpg")
        if comic_info.getboolean(
            section, "Overwrite existing images"
...
def get_storylines(
    comic_data_dicts: List[Dict], show_uncategorized: bool
) -> OrderedDict:
...
def write_html_files(
    ...
    utils.jinja_environment = Environment(
        loader=FileSystemLoader(template_folders), autoescape=True
    )
...
def write_tagged_pages(comic_data_dicts: List[Dict], global_values: Dict):
    ...
    for tag, pages in tags.items():
        html_path = os.path.join("build", "tagged", tag, "index.html")
        data_dict = {"tag": tag, "tagged_pages": pages}
        data_dict.update(global_values)
        utils.write_to_template("tagged", html_path, data_dict)
...
def print_processing_times(processing_times: List[Tuple[str, float]]):
    ...
    print(
        "{}: {:.2f} ms".format(
            "Total time", (processing_times[-1][1] - processing_times[0][1]) * 1000
        )
    )
