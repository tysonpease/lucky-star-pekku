...
{% extends "base.tpl" %}
{# This is the start of the `content` block. It's part of the body of the page. This is where all the visible parts of
the website after the links bar and before the "Powered by comic_git" footer go. #}
{% block content %}
<div id="jump-to">
    {%- if storylines.keys() | list != ["Uncategorized"] %}
    <h2>Jump to...</h2>
    {%- endif %}
    {# For loops let you take a list of a values and do something for each of those values. In this case, it runs
    through list of all the storylines in the comic (Chapter 1, Chapter 2, etc.) it generates a link for each of
    those them connecting to the first page in that storyline. #}
    ...
    <a class="chapter-links" href="#{{ pages[0].page_name }}" id="infinite-scroll-{{ name | replace(' ', '-') }}">{{
        name }}</a>
    {%- endif %}
    {%- endfor %}
</div>
...
<div id="loading-infinite-scroll">
    <p>Loading comics...</p>
</div>
...
{% block script %}
<script type="module" src="/src/js/infinite_scroll.js"></script>
{% endblock %}